...
<div class="login-container">
  <el-form class="login-form" autoComplete="on" :model="loginForm" :rules="loginRules" ref="loginForm" label-position="left">
    <div class="title-container">
      <h3 class="title">微讯助手</h3>
      <h4 class="copyright">© 啦咔网络</h4>
    </div>
    ...
    <el-button type="primary" style="width:100%;margin-bottom:30px;" size="large" :loading="loading" @click.native.prevent="handleLogin">登录</el-button>
    <el-alert v-if="emsg" :title="emsg" type="error"></el-alert>
  </el-form>
</div>
</template>
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('LoginByUsername', this.loginForm).then(() => {
            this.loading = false
            let p = Cookies.get('LAST_ROUTE_PATH')
            if (!p) {
              p = '/'
            }
            this.$router.push({
