    ...
    <div class="app-container calendar-list-container">
      <el-button class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">刷新</el-button>
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%;margin-top:15px;">
            ...
            <el-table-column width="350px" label="title" prop="title"></el-table-column>
            <el-table-column width="350px" label="order_id" prop="order_id"></el-table-column>
    ...
    export default {
        ...
        methods: {
            getList () {
                ...
                request({ url: 'taobao/soldOut', method: 'post', params: this.listQuery }).then(response => {
            ...
            showSoldAccountDetail(row) {
                request({ url: 'taobao/sold-out-account-detail', method: 'post', params: {id:row.id} }).then(response => {
                    const result = response.data;
                    if(result.code) {
                        this.$message.error(result.msg || '系统错误')
                        this.listLoading = false
