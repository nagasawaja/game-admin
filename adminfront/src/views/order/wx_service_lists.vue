...
<div class="app-container calendar-list-container">
  ...
  <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%">
    ...
    <el-table-column width="150px" label="service_product_id" prop="service_product_id"></el-table-column>
    <el-table-column width="100px" label="user_id" prop="user_id"></el-table-column>
    <el-table-column width="200px" label="wxid" prop="wxid"></el-table-column>
    <el-table-column width="150px" label="start_time" prop="start_time">
        <template slot-scope="scope">{{scope.row.start_time | formatTime('{y}-{m}-{d} {h}:{i}')}}</template>
    </el-table-column>
    <el-table-column width="150px" label="end_time" prop="end_time">
        <template slot-scope="scope">{{scope.row.end_time | formatTime('{y}-{m}-{d} {h}:{i}')}}</template>
    </el-table-column>
  </el-table>
...
export default {
  ...
  methods: {
    getList () {
      ...
      request({ url: 'order/service-order', method: 'get', params: this.listQuery }).then(response => {
        ...
        this.list.map((v) => { v.price = (v.price * 0.01).toFixed(2) })
